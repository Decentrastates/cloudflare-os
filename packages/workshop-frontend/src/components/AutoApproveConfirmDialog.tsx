        ...
        <div className="flex items-start justify-between gap-4 border-b border-kumo-line px-5 py-4">
          <div className="min-w-0">
            <Dialog.Title className="text-[15px] leading-5 font-medium tracking-[-0.3px] text-kumo-default">
              {t("Always approve “")}{actionLabel}”?
            </Dialog.Title>
            <Dialog.Description className="mt-1 text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
              {t("Future")}<span className="font-medium text-kumo-default">{actionLabel}</span> {t("actions on")}{' '}
              <span className="font-medium text-kumo-default">{resourceTitle}</span> {t("will be applied automatically, without asking for approval. This action will be applied now too.")}</Dialog.Description>
          </div>
          <Dialog.Close
            render={(props) => (
              <WorkshopIconButton
