    ...
    <div className="flex flex-col h-full bg-kumo-base">
      ...
      <div className="flex-1 overflow-auto p-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <Text variant="heading2" as="h1">{t("Channel Summarizer")}</Text>
            <p className="text-sm text-kumo-subtle mt-1">
              {t("Daily digest of your Slack channels, powered by Workers AI")}</p>
          </div>
            ...
            >
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  ...
                  <Badge variant="primary">{row.messages} {t("msgs")}</Badge>
                </div>
              ...
              <div className="space-y-1.5 mt-3">
                <div className="flex items-start gap-2">
                  ...
                  <p className="text-sm text-kumo-subtle">
                    {row.channel === '#general'
                      ? t("Team discussed Q1 planning timeline and agreed on March 15 deadline for proposals")
                      : row.channel === '#engineering'
                        ? t("Deployed v2.4.1 hotfix for auth timeout. Monitoring dashboards show latency back to normal")
                        : t("Active discussion about weekend hackathon projects and lunch plans for Friday")}
                  </p>
                </div>
                <div className="flex items-start gap-2">
                  <Circle size={5} className="text-kumo-subtle mt-1.5 flex-shrink-0" weight="fill" />
                  <p className="text-sm text-kumo-subtle">
                    {row.channel === '#general'
                      ? t("3 action items assigned, 2 decisions made")
                      : row.channel === '#engineering'
                        ? t("RFC for new caching layer received 5 approvals, moving to implementation")
                        : t("12 participants, trending topics: hackathon, team lunch, offsite")}
                  </p>
                </div>
              </div>
            </div>
