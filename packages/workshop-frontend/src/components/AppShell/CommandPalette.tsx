...
export default function CommandPalette({
  ...
  const { groups, flat } = useMemo(() => {
    ...
    const formatCommands: Command[] = formats.map((format) => ({
      ...
      label: t('New {{format}}', { format: format.output.noun }),
      hint: t('Format'),
    ...
    const wsBase: Command[] = gadgets
      ...
      .map((g) => ({
        id: `ws-${g.id}`,
        label: g.title || t('Untitled workspace'),
        hint: t('Workspace'),
    ...
    const built: Group[] = searching
      ? [
          { heading: t('Actions'), items: refine(nav, nav.length) },
          { heading: t('Workspaces'), items: refine(wsBase, 8) },
          { heading: t('Blueprints'), items: refine(bpBase, 8) },
        ]
      : [
          { heading: t('Actions'), items: refine(nav, nav.length) },
          { heading: t('Recent workspaces'), items: refine(wsBase, 4) },
        ]
    ...
    >
      ...
      <div className="themed-floating-shadow-lg relative w-full max-w-xl overflow-hidden rounded-xl border border-kumo-line bg-kumo-base">
        ...
        <div ref={listRef} className="sidebar-scroll max-h-[min(60vh,420px)] overflow-y-auto p-1.5">
          {flat.length === 0 ? (
            <p className="px-3 py-6 text-center text-[13px] text-kumo-inactive">{t("No results.")}</p>
          ) : (
            groups.map((group, gi) => {
        ...
        <div className="flex items-center gap-3 border-t border-kumo-line px-3.5 py-2 text-[11px] text-kumo-inactive">
          ...
          <span className="flex items-center gap-1">
            ...
            {t("open")}</span>
          <span className="flex items-center gap-1">
            <kbd className="rounded border border-kumo-line px-1 py-0.5 font-sans leading-none">{t("esc")}</kbd>
            {t("close")}</span>
        </div>
      </div>
    </div>
