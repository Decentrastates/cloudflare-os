...
export default function BlueprintModal({ open, onClose, overseer, gadget, metadata }: Props) {
  ...
  const loadBlueprints = useCallback(async () => {
    ...
    try {
      ...
      toasts.add({ title: t("Failed to load blueprints"), variant: 'error' })
    } finally {
      setLoading(false)
    }
  }, [overseer])
  ...
  const loadBindings = useCallback(async () => {
    ...
    try {
      ...
      setBindingsError(t("Could not load connections."))
    } finally {
  ...
  const handleScreenshotSelected = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file.type.startsWith('image/')) {
      toasts.add({ title: t("Please select an image file."), variant: 'error' })
      return
    }
    ...
    try {
      ...
      toasts.add({ title: t("Failed to process screenshot"), variant: 'error' })
    } finally {
  ...
  const createBlueprint = async () => {
    ...
    try {
      ...
      toasts.add({ title: t("Blueprint created."), variant: 'success' })
      setFormMode('list')
      setNewTitle(metadata.title)
      setNewDescription('')
      setNewScreenshotBlob(null)
      ...
      setCreateError(err.message || t("Could not create blueprint."))
    } finally {
      setCreating(false)
    }
  }
  ...
  const saveBlueprintEdits = async () => {
    ...
    try {
      ...
      toasts.add({ title: t("Blueprint updated."), variant: 'success' })
      setFormMode('list')
      ...
      setCreateError(err.message || t("Could not update blueprint."))
    } finally {
      setCreating(false)
    }
  }
  ...
  const deleteBlueprint = async (id: string) => {
    ...
    try {
      ...
      toasts.add({ title: err.message || t("Failed to delete blueprint."), variant: 'error' })
    } finally {
      setDeletingId(null)
    }
  }
          ...
          <div className="flex items-start justify-between gap-4 border-b border-kumo-line px-4 py-5 sm:px-6">
            <div className="flex min-w-0 items-start gap-3">
              ...
              <Dialog.Title className="text-[17px] leading-6 font-medium tracking-[-0.35px] text-kumo-default">
                {formMode === 'create' ? t("Create blueprint") : formMode === 'edit' ? t("Edit blueprint") : t("Blueprints")}
              </Dialog.Title>
              <Dialog.Description className="mt-1 text-[13px] leading-[18px] font-normal tracking-[-0.25px] text-kumo-subtle">
                {formMode === 'create'
                  ? t("Describe what people get when they start from this blueprint.")
                  : formMode === 'edit'
                    ? t("Update the details, screenshot, and connection guidance for this blueprint.")
                    : t("Turn this gadget into a reusable starting point.")}
              </Dialog.Description>
              </div>
            </div>
            <Dialog.Close
          ...
          >
                ...
                <div className="flex-1 overflow-y-auto chat-panel space-y-5 px-4 py-5 sm:px-6">
                  <div className="space-y-3">
                    ...
                    <WorkshopInputArea
                      ...
                      aria-label={t("Blueprint description")}
                    ...
                    <div className="rounded-xl border border-kumo-line bg-kumo-base p-3">
                      <div className="flex items-center justify-between gap-3">
                        <div className="min-w-0">
                          ...
                          <p className="m-0 mt-0.5 text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
                            {t("Optional image shown on Explore and the blueprint detail page.")}{formMode === 'edit' && !newScreenshotUrl && editingBlueprint?.screenshotUrl && !clearScreenshot ? t("The current screenshot will stay unless you upload a new one.") : ''}
                          </p>
                        </div>
                        <div className="flex shrink-0 items-center gap-1.5">
                          ...
                          >
                            ...
                            {processingScreenshot ? t("Processing...") : newScreenshotUrl || (formMode === 'edit' && editingBlueprint?.screenshotUrl && !clearScreenshot) ? t("Change") : t("Upload")}
                          </WorkshopButton>
                        </div>
                      </div>
                      {screenshotPreviewUrl && (
                        <div className="mt-3 overflow-hidden rounded-lg border border-kumo-line bg-kumo-tint">
                          <img
                            ...
                            alt={t("Blueprint screenshot preview")}
                      ...
                      {clearScreenshot && !newScreenshotUrl && (
                        <div className="mt-3 rounded-lg border border-dashed border-kumo-line bg-kumo-tint px-3 py-2 text-[12px] leading-4 text-kumo-subtle">
                          {t("Screenshot will be removed when you save.")}</div>
                      )}
                    </div>
                  </div>
                  ...
                  {bindingsLoading ? (
                    ...
                    <section>
                      ...
                      <p className="m-0 mb-3 text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
                        {t("Name each connection and add guidance for people using this blueprint.")}</p>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    >
                      {creating
                        ...
                        : processingScreenshot ? t("Processing...")
                          : bindingsLoading ? t("Loading...")
            ...
            <section>
              <h3 className="mb-2 text-[13px] leading-[18px] font-medium tracking-[-0.25px] text-kumo-default">
                {t("Existing blueprints")}</h3>

              {loading ? (
                <div className="rounded-xl border border-kumo-line bg-kumo-base px-4 py-6 text-center text-[13px] text-kumo-subtle">
                  {t("Loading blueprints...")}</div>
              ) : blueprints.length === 0 ? (
                    ...
                    <BlueprintRow
                      ...
                      onUpdateCode={async () => {
                        try {
                          ...
                          toasts.add({ title: t("Blueprint updated to current code."), variant: 'success' })
                          loadBlueprints()
                        } catch (err: any) {
                          toasts.add({ title: err.message || t("Failed to update blueprint."), variant: 'error' })
                        }
                      }}
                      onRetryPublish={async () => {
                        try {
                          await overseer.retryBlueprintPublish(bp.id)
                          toasts.add({ title: t("Blueprint published successfully."), variant: 'success' })
                          loadBlueprints()
                        } catch (err: any) {
                          toasts.add({ title: err.message || t("Retry failed."), variant: 'error' })
                        }
                      }}
                      onCopyLink={async () => {
      ...
      >
        <div className="flex w-full flex-wrap items-center gap-3">
          <div className="min-w-0 flex-1">
            ...
            <p className="m-0 mt-0.5 text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
              {t("People who started a gadget from this blueprint won't be affected, but the link will stop working.")}</p>
          </div>
          <button
            type="button"
    ...
    >
      <div className="flex min-w-0 flex-wrap items-center gap-3">
        ...
        <span
          ...
          title={bp.dirty ? t('Last publish failed') : undefined}
        >
          {bp.dirty && (
            <span
          ...
          {t("v")}{bp.version} · {new Date(bp.codeVersionDate).toLocaleDateString()}
        </span>
      </div>
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
        <div className="-ml-[7px] flex flex-wrap items-center gap-1">
          ...
          >
            {copyState === 'copied' ? t("Copied") : copyState === 'failed' ? t("Copy failed") : t("Copy link")}
          </GhostButton>
        </div>
