...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    const fetchUserInfo = async () => {
      try {
        ...
        if (!cancelled) toasts.add({ title: t("Failed to load user information"), variant: 'error' })
      } finally {
  ...
  const handleSaveName = async () => {
    if (!nameInput.trim()) {
      toasts.add({ title: t("Display name cannot be empty"), variant: 'error' })
      return
    }
    ...
    try {
      ...
      toasts.add({ title: t("Display name updated"), variant: 'success' })
    } catch (err) {
      console.error('Failed to update display name:', err)
      toasts.add({ title: t("Failed to update display name"), variant: 'error' })
    }
  }
  ...
  const handleCopyId = async () => {
    ...
    try {
      ...
      toasts.add({ title: t("User ID copied"), variant: 'success' })
    } catch {
      toasts.add({ title: t("Failed to copy"), variant: 'error' })
    }
  }
  ...
  const handleAvatarUpload = async (file: File) => {
    ...
    try {
      ...
      toasts.add({ title: t("Avatar updated"), variant: 'success' })
    } catch (err) {
      console.error('Failed to upload avatar:', err)
      setLocalAvatarPreview(null)
      toasts.add({ title: t("Failed to upload avatar"), variant: 'error' })
    } finally {
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      toasts.add({ title: t("Password changed successfully"), variant: 'success' })
      setCurrentPassword('')
      setNewPassword('')
      setConfirmPassword('')
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : t('Failed to change password')
  ...
  if (loading) {
    return (
      <div className="flex min-h-[60vh] flex-1 items-center justify-center">
        <p className="text-[13px] tracking-[-0.25px] text-kumo-subtle">{t("Loading profile…")}</p>
      </div>
    )
  }
  ...
  return (
    <div className="mx-auto flex h-full w-full max-w-2xl flex-col px-6 pb-16 sm:px-10">
      <header className="px-1 pb-2 pt-10">
        <h1 className="text-2xl font-semibold tracking-tight text-kumo-default">{t("Profile")}</h1>
        <p className="mt-1 text-[13px] leading-[18px] tracking-[-0.25px] text-kumo-subtle">
          {t("Manage your account details, avatar, and security.")}</p>
      </header>

      <div className="mt-6 flex flex-col gap-9">
        <section className="flex flex-col gap-3">
          <SectionLabel>{t('Language')}</SectionLabel>
          <div className="flex items-center justify-between rounded-xl border border-kumo-line bg-kumo-base px-5 py-4">
            ...
            <div className="flex items-center gap-4 px-5 py-4">
              ...
              <div className="min-w-0">
                ...
                <p className="mt-0.5 text-[12px] leading-4 tracking-[-0.2px] text-kumo-subtle">
                  {t("Click the avatar to upload a new photo")}</p>
              </div>
            </div>
