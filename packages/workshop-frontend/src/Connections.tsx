...
export default function Connections({ overseer, gadget, chatId, authenticatedApi, onConnectionsChange, isVisible, onHasGatekeepersChange }: ConnectionsProps) {
  ...
  const loadGatekeepers = async () => {
    try {
      ...
      toasts.add({ title: t("Failed to load connections"), variant: 'error' })
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleDeleteHookConfirm = async () => {
    ...
    try {
      ...
      toasts.add({ title: t("Failed to delete hook"), variant: 'error' })
    } finally {
  ...
  const handleEditSave = async (name: string) => {
    ...
    if (!newName) {
      toasts.add({ title: t("Binding name cannot be empty"), variant: 'error' })
      return
    }
    if (newName === name) {
    ...
    try {
      ...
      toasts.add({ title: t("Failed to update binding name"), variant: 'error' })
    } finally {
      setEditingBinding(null)
    }
  }
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      toasts.add({ title: t("Failed to remove connection"), variant: 'error' })
    } finally {
  ...
  return (
    <div className="h-full overflow-auto bg-kumo-base">
          ...
          <div className="mb-3 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div className="min-w-0">
              ...
              <p className="mt-1 text-[13px] leading-[18px] font-normal tracking-[-0.25px] text-kumo-subtle">
                {t("External resources this gadget can use.")}</p>
            </div>
            <WorkshopButton
              tone="primary"
            ...
            >
              {t("Connect resource")}</WorkshopButton>
          </div>
                      ...
                      <div className="flex flex-wrap items-center gap-3">
                        <div className="min-w-0 flex-1">
                          <p className="truncate text-[13px] leading-[18px] font-medium tracking-[-0.25px] text-kumo-danger">
                            {t("Delete")}{gk.resourceTitle}?
                          </p>
                          <p className="truncate text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
                            {t("The binding")}<span className="font-mono">{gk.name}</span> {t("will be removed from this gadget.")}</p>
                        </div>
                        <WorkshopButton
                          tone="danger"
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="mt-0.5 truncate text-[11px] leading-4 tracking-[-0.1px] text-kumo-inactive">
                            {t("Referenced in code as:")}<span className="font-mono text-kumo-subtle">{gk.name}</span>
                          </p>
                        </div>
          ...
          <section className="mt-8">
            <div className="mb-3">
              <h2 className="m-0 text-[17px] leading-6 font-medium tracking-[-0.35px] text-kumo-default">
                {t("Hooks")}</h2>
              <p className="mt-1 text-[13px] leading-[18px] font-normal tracking-[-0.25px] text-kumo-subtle">
                {t("Callbacks that let connected resources wake up this gadget when events happen.")}</p>
            </div>
                  ...
                  >
                    {isDeleting ? (
                      <div className="flex flex-wrap items-center gap-3">
                        <div className="min-w-0 flex-1">
                          <p className="truncate text-[13px] leading-[18px] font-medium tracking-[-0.25px] text-kumo-danger">
                            {t("Delete hook \"")}{hook.description.title}"?
                          </p>
                          <p className="truncate text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
                            {t("This permanently removes the hook. Future events will stop being delivered.")}</p>
                        </div>
                        <WorkshopButton
                          tone="danger"
                          className="min-w-[68px]"
          ...
          try {
            ...
            toasts.add({
              title: chatId === undefined
                ? t("Connection created successfully")
                : t("Connection created — accept the chat's changes to keep it"),
              variant: 'success',
            })
            await loadGatekeepers()
            onConnectionsChange?.()
      ...
      <BlueprintAnnotationModal
        ...
        onSaved={() => {
          toasts.add({ title: t("Blueprint settings saved."), variant: 'success' })
...
function BlueprintAnnotationModal({
      ...
      try {
        ...
        if (!cancelled) {
          if (loaded) {
            ...
            setLoadError(t("Connection not found."))
          }
        }
      } catch (err: any) {
        if (!cancelled) {
          reportIssue('connections.binding-load', err)
          setLoadError(err?.message || t("Could not load binding."))
        }
      }
    })()
    return () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setSaveError(err?.message || t("Could not save."))
    } finally {
      setSaving(false)
    }
  }
        ...
        <Dialog className="!z-[1000] !w-[min(480px,calc(100vw-32px))] overflow-hidden bg-kumo-base p-0" size="lg">
          <div className="flex items-start justify-between gap-4 border-b border-kumo-line px-4 py-4 sm:px-5">
            <div className="min-w-0">
              <Dialog.Title className="text-[15px] leading-5 font-medium tracking-[-0.3px] text-kumo-default">
                {t("Blueprint settings")}</Dialog.Title>
              <Dialog.Description className="mt-1 text-[12px] leading-4 font-normal tracking-[-0.2px] text-kumo-subtle">
                {t("How this connection appears in blueprints.")}</Dialog.Description>
            </div>
            <Dialog.Close
              render={(props) => (
          ...
          <div className="space-y-4 px-4 py-4 sm:px-5">
            {loadError ? (
              ...
              <div className="py-2 text-center text-[13px] text-kumo-subtle">{t("Loading...")}</div>
            ) : (
              <>
                <BlueprintBindingCard
                  data={data}
