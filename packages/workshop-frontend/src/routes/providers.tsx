          ...
          <DropdownMenu.Content className={MENU_CONTENT}>
            ...
            {!isBuiltIn && (
              <DropdownMenu.Item variant="danger" onClick={onDelete} className={MENU_ITEM_DANGER}>
                ...
                {t("Delete provider")}</DropdownMenu.Item>
            )}
...
function ProvidersPage() {
  useDocumentTitle(t("AI Providers"))
  ...
  const handleDelete = async (model: AiChatAuthorInfo) => {
    if (!confirm(t('Delete "{{name}}"? This cannot be undone.', { name: model.name }))) return
    ...
    try {
      ...
      toasts.add({ title: t("Failed to delete provider"), variant: 'error' })
    } finally {
      setDeletingId(null)
    }
  }
  ...
  const handleSetQuick = async (modelId: string) => {
    ...
    try {
      ...
      toasts.add({ title: t("Failed to update default model"), variant: 'error' })
    }
  }
    ...
    <div className="mx-auto flex h-full w-full max-w-4xl flex-col px-6 sm:px-10">
      <header className="flex items-end justify-between gap-4 px-3 pb-3 pt-10">
        <div className="min-w-0">
          <h1 className="text-2xl font-semibold tracking-tight text-kumo-default">{t("AI providers")}</h1>
          <p className="mt-1 text-[13px] leading-[18px] tracking-[-0.25px] text-kumo-subtle">
            {t("Configure the AI models available to your workspaces.")}</p>
        </div>
        <button type="button" onClick={() => setSheetOpen(true)} className={PRIMARY_BTN}>
          <Plus size={14} weight="bold" />
          {t("Add provider")}</button>
      </header>
      ...
      <div className="chat-panel flex min-h-0 flex-1 flex-col gap-0.5 overflow-y-auto pt-1 pb-16">
          ...
          <div className="flex flex-col gap-2.5 px-3 pb-2">
            {gatewayMode && (
              <Notice>
                ...
                <span>
                  <strong className="font-medium text-kumo-default">{t("AI Gateway mode:")}</strong> {t("built-in models are managed by your deployment. You can still add custom models with your own API tokens.")}</span>
              </Notice>
            )}
            ...
            {!gatewayMode && models.length > 0 && (
              <Notice>
                <Lightning size={15} className="mt-px shrink-0 text-kumo-brand" />
                <span>
                  <strong className="font-medium text-kumo-default">{t("Quick model:")}</strong>{' '}
                  {quickModel
                    ? t('{{name}}.', { name: models.find((m) => m.id === quickModel)?.name ?? quickModel })
                    : t("none set.")}{' '}
                  {t("Used for fast tasks like generating chat titles. Click a model to set it.")}</span>
              </Notice>
            )}
          </div>
        )}
        ...
        {loading ? (
          ...
          <div className="py-12 text-center text-sm">
            <p className="text-kumo-danger">{t("Something went wrong loading your providers.")}</p>
          ...
          <div className="flex flex-col items-center gap-3 px-3 py-16 text-center">
            ...
            <div>
              <p className="text-sm font-medium text-kumo-default">{t("No AI providers yet")}</p>
              <p className="mt-1 text-[13px] leading-[18px] text-kumo-subtle">
                {t("Add a provider to start building workspaces with AI.")}</p>
            </div>
            <button type="button" onClick={() => setSheetOpen(true)} className={PRIMARY_BTN}>
              <Plus size={14} weight="bold" />
              {t("Add your first provider")}</button>
          </div>
        ) : filtered.length === 0 ? (
          <div className="py-12 text-center text-sm text-kumo-inactive">{t("No providers found")}</div>
        ) : (
          filtered.map((model) => (
            <div
              key={model.id}
