...
export default function WorkpiecePicker({
  ...
  return (
    ...
    >
      ...
      >
        {expanded && (
          <span className="text-[11px] font-medium uppercase tracking-[0.06em]">{t("Outputs")}</span>
        )}
      ...
      <div className="flex flex-1 flex-col gap-0.5 overflow-y-auto overflow-x-hidden px-1.5 pb-2">
        {gadgets.map(gadget => {
          ...
          return (
            ...
            >
              <Tooltip content={`${gadget.title}${!expanded && isPending ? ` (${t('Draft')})` : ''}${hasHook ? ` · ${t('Hooks enabled')}` : ''}`} asChild>
                <button
                  type="button"
              ...
              {expanded && (
                <WorkshopIconButton
                  ...
                  aria-label={t('Rename {{title}}', { title: gadget.title })}
                >
        ...
        <Tooltip content={t('View activity')} asChild>
          ...
          >
            ...
            {expanded && <span className="min-w-0 flex-1 truncate">{t("View activity")}</span>}
            {expanded ? (
