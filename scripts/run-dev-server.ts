...
for (const gk of gatekeepers) {
  ...
  if (enableDevWatchers && existsSync(join(gk.dir, "src", "configurator"))) {
    spawnDevWatcher(
  ...
  if (enableDevWatchers && existsSync(join(gk.dir, "build-app.mjs"))) {
...
for (const gk of gatekeepers) {
  ...
  config.vars.BASE_URL = getGatekeeperBaseUrl(process.env.PUBLIC_BASE_URL, gk.name) ??
      `http://${backendHost}/gatekeeper/${gk.name.slice("gatekeeper-".length)}`;
...
// ---------------------------------------------------------------------------
{
  ...
  // with a JSON array so a predictable public username is never privileged.
  config.vars = config.vars || {};
  config.vars.ADMINS = getConfiguredAdmins(process.env.CLOUDFLARE_OS_ADMINS);
