...
/** Build the externally reachable callback base for one gatekeeper. */
...
/** Resolve the self-hosted admin allowlist without granting a public default account. */
export function getConfiguredAdmins(configuredAdmins: string | undefined): string[] {
  if (configuredAdmins === undefined) return ["admin"];
  ...
  if (!Array.isArray(admins) ||
      admins.some(name => typeof name !== "string" || !name)) {
