...
describe("getGatekeeperBaseUrl", () => {
  it("maps a public URL to a gatekeeper callback origin", () => {
    assert.equal(
        getGatekeeperBaseUrl("https://cloudflare-os.cddao.com/", "gatekeeper-github"),
        "https://cloudflare-os.cddao.com/gatekeeper/github");
  });

  it("keeps local defaults when no public URL is configured", () => {
    assert.equal(getGatekeeperBaseUrl(undefined, "gatekeeper-google"), null);
  });
});

describe("getConfiguredAdmins", () => {
  it("keeps the local development default", () => {
    assert.deepEqual(getConfiguredAdmins(undefined), ["admin"]);
  });

  it("uses the self-hosted allowlist", () => {
    assert.deepEqual(getConfiguredAdmins('["operator"]'), ["operator"]);
  });

  for (const value of ["not-json", "{}", '[""]']) {
    it(`rejects invalid allowlists: ${value}`, () => {
      assert.throws(() => getConfiguredAdmins(value), /CLOUDFLARE_OS_ADMINS/);
    });
  }
});
