...
import {
  embeddedUiZhCN,
  translateEmbeddedUi,
} from "../packages/workshop-shared/src/embedded-ui-i18n.ts";
...
const embeddedCatalogs = ${JSON.stringify({
  'zh-CN': embeddedUiZhCN,
  'zh-TW': Object.fromEntries(Object.keys(embeddedUiZhCN).map((key) => [
    key, translateEmbeddedUi('zh-TW', key),
  ])),
})};
let locale = "en";
function t(message) { return embeddedCatalogs[locale]?.[message] ?? message; }
...
const components = {
  Section({ title = null, children }) {
    return el("section", { className: "section" }, [
      title ? el("h2", { className: "section-title", text: t(title) }) : null,
  ...
  Field({ label, description, optional, children }) {
    return el("section", { className: "field" }, [
      el("div", { className: "field-header" }, [
        el("label", { className: "field-label", text: t(label) }),
        optional ? el("span", { className: "field-optional", text: t("Optional") }) : null,
      ]),
      description ? el("p", { className: "field-description", text: t(description) }) : null,
  ...
  RadioCards({ value, options, onChange }) {
    return el("div", { className: "radio-cards" }, options.map(option => {
      ...
      return el("button", { type: "button", className: selected ? "radio-card selected" : "radio-card", onclick: () => onChange(option.value) }, [
        el("span", { className: "radio-title", text: t(option.title) }),
        el("span", { className: "radio-description", text: t(option.description) }),
      ]);
    }));
  },
  ...
  CheckboxList({ name, value, loadOptions, onChange, allSelected, disabled }) {
    ...
    if (status === "loading") return notice(t("Loading..."));
    if (status === "failed") return notice(message);
    const shownOptions = withUnavailableOptions(options, value);
    if (shownOptions.length === 0) return notice(t("This server published no tools."));
    ...
    if (matches.length > 0 && !allMatchesSelected) {
      bulk.push(el("button", {
        ...
        disabled,
        text: needle ? \`${'${'}t("Select")} \${matches.length} ${'${'}t("shown")}\` : t("Select all"),
        onclick: () => {
    ...
    if (clearable.length > 0) {
      bulk.push(el("button", {
        ...
        className: "checkbox-action",
        disabled,
        text: needle ? \`${'${'}t("Clear")} \${clearable.length} ${'${'}t("shown")}\` : t("Clear"),
        onclick: () => {
          const next = new Set(selected);
    ...
    children.push(el("div", { className: "checkbox-toolbar" }, [
      el("span", {
        ...
        text: selected.size > 0
          ? \`\${selected.size} ${'${'}t("of")} \${shownOptions.length} ${'${'}t("selected")}\`
          : \`${'${'}t("None of")} \${shownOptions.length} ${'${'}t("selected")}\`,
      }),
      bulk.length > 0 ? el("span", { className: "checkbox-actions" }, bulk) : null,
    ]));

    if (matches.length === 0) {
      children.push(el("p", { className: "checkbox-empty", text: \`${'${'}t("Nothing matches")} "\${filter.trim()}".\` }));
  ...
  Autocomplete({ name, value, placeholder, loadOptions, onChange, optional, onClear, disabled }) {
    ...
    function renderPopup({ loading = false, error = null, options = [] }) {
      ...
      applyPopupMaxHeight();
      if (loading) {
        popup.append(el("div", { className: "autocomplete-empty", text: t("Loading...") }));
      } else if (error) {
        popup.append(el("div", { className: "autocomplete-empty", text: error }));
      } else if (options.length === 0) {
        popup.append(el("div", { className: "autocomplete-empty", text: t("No matches.") }));
      } else {
    ...
    async function refreshNow() {
      ...
      try {
        ...
        renderPopup({ error: error?.message || t("Could not load options.") });
      }
    }
...
async function main() {
  ...
  ui = host.gatekeeper;
  try { locale = await host.getLocale(); } catch {}
